...
# Create AmneziaWG configuration directory
RUN mkdir -p /etc/amnezia/amneziawg

# Install build dependencies temporarily and build AmneziaWG tools
...
# Update healthcheck to handle when awg interface isn't running
HEALTHCHECK --interval=1m --timeout=5s --retries=3 CMD /usr/bin/timeout 5s /bin/sh -c "/usr/bin/awg show 2>/dev/null | /bin/grep -q interface || /bin/true"
